...
Quality & Operationsコンポーネントは、「定義されたスキーマに対して集約出力を検証し、無効なデータがワークフローを通じて伝播するのを防ぐ」機能を提供する。Agents SDKでは、OutputGuardrailと`on_handoff`コールバックのPydanticバリデーション（`input_type`パラメータ）がこの役割を担う。
