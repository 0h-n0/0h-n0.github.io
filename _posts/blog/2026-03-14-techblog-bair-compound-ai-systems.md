...
BAIRブログで紹介されているCompound AIパターン（RAG + モデルカスケード + コード実行）をAWSで実現する構成。
...
| **Medium** | ~30,000 (1,000/日) | Hybrid | $500-1,500 | ECS + OpenSearch + ElastiCache |
| **Large** | 300,000+ (10,000/日) | Container | $3,000-8,000 | EKS + OpenSearch + Bedrock Batch |

**Small構成の詳細**（月額$80-200）:
- **Lambda**: パイプラインオーケストレーション（$15/月）
- **Bedrock**: Claude Haiku（ルーティング）+ Claude Sonnet（推論）（$100/月）
- **OpenSearch Serverless**: RAG用ベクトル検索（$50/月）
- **S3**: ドキュメントストレージ（$5/月）

**コスト削減テクニック**:
- FrugalGPTパターン（モデルカスケード）でBedrock呼び出しコスト60-90%削減
- OpenSearch Serverlessのオートスケーリングでアイドルコスト最小化
- Prompt Caching有効化で繰り返しクエリのコスト30-90%削減

**コスト試算の注意事項**: 上記は2026年3月時点のAWS ap-northeast-1料金に基づく概算値です。最新料金は[AWS料金計算ツール](https://calculator.aws/)で確認してください。

### コスト最適化チェックリスト

- [ ] モデルカスケード（Haiku→Sonnet→Opus）で段階的にコスト配分
- [ ] RAG検索のヒット率監視（低ヒット率はコスト増大の兆候）
- [ ] OpenSearch Serverlessのインデックス最適化（不要なフィールド削除）
