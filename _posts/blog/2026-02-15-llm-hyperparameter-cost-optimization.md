---
layout: post
title: "論文解説: LLM生成パラメータのコスト最適化: ベイズ最適化で20-40%削減"
description: "temperature、top_p、max_tokensなどの生成ハイパーパラメータをベイズ最適化で調整し、品質制約を満たしつつコストを20-40%削減。要約タスクでは「temperature=0.3, top_p=0.85, max_tokens=150」が最適解。50-100試行で最適化コストは"
...
**タイトル**: LLM生成パラメータのコスト最適化: ベイズ最適化で20-40%削減
**arXiv ID**: [2311.09668](https://arxiv.org/abs/2311.09668)
**対象読者**: 修士学生レベル（機械学習・LLMの基礎知識を持つエンジニア・研究者）

temperature、top_p、max_tokensなどの生成ハイパーパラメータをベイズ最適化で調整し、品質制約を満たしつつコストを20-40%削減。要約タスクでは「temperature=0.3, top_p=0.85, max_tokens=150」が最適解。50-100試行で最適化コストは$50-500だが、日次10K以上のリクエストで投資回収可能。品質メトリック(BLEUなど)が人間評価と乖離する場合は適用不可。
...
- [arXiv:2311.09668](https://arxiv.org/abs/2311.09668)
