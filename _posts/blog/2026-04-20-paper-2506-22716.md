...
1. **Value model依存**: BEST-Routeの効果はvalue/reward modelの品質に直接依存する。特定のタスクやドメインに対するvalue modelの構築には、ラベル付きデータの収集と追加の計算コストが必要である
2. **2モデル前提**: 現行の定式化は主にbinary routing（小モデル vs. 大モデル）を対象としている。3つ以上のモデル階層への拡張は将来課題として残されている
3. **ベンチマーク範囲の偏り**: 評価が数学・コード・選択式問題に集中しており、オープンエンドの生成タスクや創造的タスクでの性能は未検証である
