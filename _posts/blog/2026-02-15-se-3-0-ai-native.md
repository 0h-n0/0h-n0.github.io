...
```mermaid
graph TB
    DEV[開発者\n意図・要件定義] --> TN[Teammate.next\nMulti-modal AI協働者\nProactive Suggestion]
    TN --> IDE[IDE.next\nConversational UI\n意図キャプチャ・洗練]
    IDE --> CC[Compiler.next\nIntent-to-Code変換\nセマンティック検証]
...
sequenceDiagram
    ...
    participant AI as Teammate.next
    participant CC as Compiler.next
    H->>AI: 初期意図 I₀（曖昧な要求）
    AI->>H: 質問 q₁（仕様の明確化）
    H->>AI: 回答 a₁
    AI->>H: 質問 q₂（制約の確認）
    H->>AI: 回答 a₂
    AI->>CC: 洗練された意図 Iₙ（実装可能）
    CC->>AI: 生成コード + セマンティック検証結果
    AI->>H: 実装提案 + 設計レビュー選択肢
    H->>AI: フィードバック
    AI->>CC: コード改善指示
    CC->>H: 最終実装
