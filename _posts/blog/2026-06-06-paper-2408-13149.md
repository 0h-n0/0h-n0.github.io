...
著者らは、成功タスクと失敗タスクでのエージェント行動パターンを分析している（論文Section 4.2より）。

**成功タスクに共通する行動シーケンス:**

1. リポジトリ構造の探索（`find`, `ls`コマンド）
2. 関連ファイルの特定（`grep`による検索）
3. 既存テストの実行による現状把握
4. 段階的なパッチ生成と検証の反復

**失敗タスクに共通する行動パターン:**

- **コンテキスト消費**: 大量のファイルを読み込み、LLMのコンテキストウィンドウを消費する。著者らはこれを「exploration without exploitation」と表現している
- **反復ループ**: 同じ修正を微修正しながら繰り返し、進捗しない。修正→テスト失敗→再修正のサイクルが5回以上続くケースが報告されている
- **早期終了**: テストを実行せずにパッチを提出し、構文エラーや論理エラーを見落とす

著者らはまた、エージェントのアクション数とタスク成功率の関係を分析し、成功タスクでは平均15-25アクション、失敗タスクでは30アクション以上を消費する傾向があると報告している。

### マルチエージェント評価

著者らは、OpenHandsプラットフォーム上で複数のエージェント設計を比較評価している。CodeActAgent以外にも、以下のエージェントが実装・評価されている。

| エージェント | アクション空間 | 特徴 |
|---|---|---|
| CodeActAgent | Pythonコード実行 | デフォルト、最高性能 |
| BrowsingAgent | ブラウザ操作 | Web情報収集特化 |
| DelegatorAgent | サブエージェント委譲 | タスク分割・協調 |

DelegatorAgentは、複雑なタスクをサブタスクに分割し、適切なエージェント（CodeActまたはBrowsing）に委譲する設計であるが、著者らの実験ではCodeActAgent単体と比較して明確な改善は確認されなかったと報告されている。
