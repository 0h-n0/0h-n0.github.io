...
co-hosting環境でTP設定を選択する際の判断基準を、ブログの報告をもとに整理する。

**ステップ1: モデルサイズとGPUメモリの適合性**

単一GPU（80GB HBM）に収まるモデルかどうかを判定する。bfloat16精度の場合、モデルパラメータ数 × 2バイトが最低限必要なvRAMである。

$$
\text{必要vRAM} = P \times 2 + \text{KVキャッシュ} + \text{一時メモリ}
$$

ここで、$P$はモデルパラメータ数（例: 8B = 80億パラメータ → 約16GB）。KVキャッシュと一時メモリを含めると、7-8Bモデルで約24-30GB、13Bモデルで約40-50GBが目安となる。

**ステップ2: ワークロードの並行度プロファイリング**

ブログの報告では、以下の目安が示されている。

| 並行度 | 推奨TP | 8GPU時のインスタンス数 | 理由 |
|-------|--------|---------------------|------|
| 1-8 req/s | TP=8 | 1 | 個々のリクエスト応答速度を最速化 |
| 10-100 req/s | TP=4 | 2 | レイテンシとスループットのバランス |
| 100-1000 req/s | TP=2 | 4 | スループット優先 |
| 1000+ req/s | TP=1 | 8 | GPU間通信を排除し最大スループット |

**ステップ3: ベンチマーク検証**

プロファイリングの結果は目安に過ぎず、実際のワークロード（入出力トークン長、バッチパターン）で検証が必要であるとブログは強調している。

### メモリパーティション比率の計算

2モデルをco-hostingする場合のメモリパーティション比率の決定方法を示す。

$$
r_i = \frac{\text{モデル}_i\text{の必要vRAM}}{\text{GPU総vRAM}} \times (1 - r_{\text{buffer}})
$$

ここで、$r_{\text{buffer}}$は一時メモリ用のバッファ比率（ブログでは0.2を推奨）。

例: 7Bモデル（24GB必要）と3Bモデル（12GB必要）を80GB GPU上でco-hostingする場合:
- $r_1 = 24/80 \times 0.8 = 0.24 \rightarrow 0.4$に切り上げ（KVキャッシュ余裕確保）
- $r_2 = 12/80 \times 0.8 = 0.12 \rightarrow 0.3$に切り上げ
- 合計: 0.7（バッファ0.3）
...
ブログおよび関連ドキュメントから得られる本番運用での典型的な障害パターンを整理する。

| 障害 | 原因 | 対処 |
|------|------|------|
| co-hosted モデルの1つがCUDA OOM | パーティション比率が不適切 | バッファ比率を0.2→0.3に拡大 |
| ホットリロード後にレイテンシ増大 | 新モデルのKVキャッシュがウォームアップされていない | リロード後にウォームアップリクエストを送信 |
| 特定モデルのスループット低下 | 他モデルの負荷集中で一時メモリが圧迫 | 負荷分散またはパーティション比率の見直し |
| コンテナ起動タイムアウト | 複数モデルの同時ロードで時間がかかる | startupProbeのタイムアウトを延長（600秒以上推奨） |
