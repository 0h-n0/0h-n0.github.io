...
## RouteLLM・FrugalGPTとのアプローチ比較

Hybrid LLMの位置づけを明確にするため、主要な先行手法との違いを整理する。

| 特徴 | FrugalGPT | RouteLLM | Hybrid LLM |
|------|-----------|----------|------------|
| ルーティング方式 | カスケード（逐次） | 1-pass（嗜好データ） | 1-pass（品質予測） |
| 品質制約 | 暗黙的 | 暗黙的 | 明示的（最適化目的関数に組込） |
| 学習データ | (q, a, 正解) | 人間嗜好ペア | (q, $q_{\text{weak}}$, $q_{\text{strong}}$) |
| 理論保証 | なし | なし | Neyman-Pearson + PAC |
| レイテンシ | 最悪 $k$ 倍 | ルーター推論のみ | ルーター推論のみ |
| 品質安全性 | 回答確認後に判断 | 事前予測のみ | 事前予測 + 品質保証 |

FrugalGPTのカスケードは回答を実際に生成してから品質判定するため安全性が高いが、レイテンシが増加する。RouteLLMは嗜好データで学習するため多様なモデルペアに適用しやすいが、品質制約を直接最適化していない。Hybrid LLMはNeyman-Pearsonの補題に基づく理論的な品質保証を提供する点で独自の強みを持つ。

## 品質スコアの定義とタスク依存性

品質スコア $q_{\text{weak}}(x)$ の具体的な定義はタスクによって異なる。

- **分類・QAタスク**: Exact MatchまたはF1スコア。正解/不正解の二値でラベルが付けられるため、ルーター学習が比較的容易
- **生成タスク**: GPT-4による品質評価（1〜5スコア）またはROUGE/BERTScore。主観的な評価が必要なため、ラベルのノイズが大きくなる
- **対話タスク**: Human preferenceラベル。人間の嗜好に基づくため最も信頼性が高いが、収集コストも最も高い

Azure AI Foundry Model Routerが内部で使用している品質推定メカニズムの詳細は公開されていないが、Hybrid LLMの品質予測アプローチが理論的基盤の一つとなっている可能性がある。
...
- **ルーター学習の初期コスト**: 学習データ収集のために一度はStrong/Weak両モデルを呼ぶ必要があり、数千〜数万クエリ規模の初期投資が発生する
