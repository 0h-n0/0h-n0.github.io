...
### AgenTracer-8Bの推論環境構成

AgenTracer-8Bモデルの推論を本番環境で運用する場合の構成例です。

| 規模 | 月間分析トレース数 | 推奨構成 | 月額コスト目安 |
|------|----------------|---------|-------------|
| **Small** | ~500 | Serverless (SageMaker Serverless) | $100-250 |
| **Medium** | ~5,000 | SageMaker Real-time (g5.xlarge) | $400-900 |
| **Large** | 50,000+ | EKS + vLLM (g5.xlarge Spot) | $1,500-3,500 |

**Small構成**: SageMaker Serverless Endpointで8Bモデルを配置。リクエストがない時間帯はコストゼロ。コールドスタート（約60秒）が許容できるバッチ分析に適しています。

**Medium構成**: SageMaker Real-time Endpoint（ml.g5.xlarge, NVIDIA A10G GPU）で常時起動。レイテンシ要件がある場合に推奨。Prompt Cachingは不要（入力はトラジェクトリテキストのため）。

**Large構成**: EKSクラスタ上でvLLMを使用して8Bモデルをサービング。Karpenter + Spot Instancesで最大90%のコスト削減。複数レプリカで高スループットを実現。

**コスト比較**: AgenTracer-8Bの推論コストは、Gemini-2.5-Pro APIコストの約1/50、Claude-4-Sonnet APIコストの約1/30と推定されます（GPU推論の固定費用 vs トークン従量課金の比較）。大量のトレース分析が必要な場合、自前モデル運用のコスト優位性が顕著です。

**コスト試算の注意事項**: 上記は2026年5月時点のAWS ap-northeast-1料金に基づく概算値です。実際のコストはトレースの長さ（入力トークン数）とバッチ処理の効率に依存します。
