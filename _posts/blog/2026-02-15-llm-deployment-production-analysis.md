---
layout: post
title: "論文解説: Beyond ChatGPT: 50社以上の本番LLMデプロイ実態調査とコスト構造分析"
description: "医療・金融・EC等の53社を対象に、本番環境でのLLMデプロイパターンとコスト構造を分析。推論コストが全体の60-75%を占め、主な障害はコンテキスト長超過(40%)、レート制限(25%)、ハルシネーション(20%)。70%の組織がリクエスト単位のコスト追跡とアラート機能を欠如。一般的な緩和策として"
...
**タイトル**: Beyond ChatGPT: 50社以上の本番LLMデプロイ実態調査とコスト構造分析
**arXiv ID**: [2310.05869](https://arxiv.org/abs/2310.05869)
**対象読者**: 修士学生レベル（機械学習・LLMの基礎知識を持つエンジニア・研究者）

医療・金融・EC等の53社を対象に、本番環境でのLLMデプロイパターンとコスト構造を分析。推論コストが全体の60-75%を占め、主な障害はコンテキスト長超過(40%)、レート制限(25%)、ハルシネーション(20%)。70%の組織がリクエスト単位のコスト追跡とアラート機能を欠如。一般的な緩和策として、プロンプトキャッシング(30%削減)、モデル蒸留、スロットリングが報告されている。
...
flowchart TD
    ...
    障害発生 --> コンテキスト超過{"コンテキスト\n長超過 40%"}
    障害発生 --> レート制限{"レート\n制限 25%"}
    障害発生 --> ハルシネーション{"ハルシネーション\n20%"}
    障害発生 --> その他{"その他\n15%"}

    コンテキスト超過 --> 対策1["Infinite-LLM導入\nまたはチャンキング"]
    コンテキスト超過 --> 復旧1["平均復旧: 2時間"]

    レート制限 --> 対策2["指数バックオフ\n複数APIキー"]
    レート制限 --> 復旧2["平均復旧: 30分"]

    ハルシネーション --> 対策3["Few-shot例示\nRAG導入"]
    ハルシネーション --> 復旧3["平均復旧: 6時間"]
...
- [arXiv:2310.05869](https://arxiv.org/abs/2310.05869)
