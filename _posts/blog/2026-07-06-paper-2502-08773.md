...
UniRouteの汎用ルーティングをAWS上で実装する場合のパターンを示す。

| 規模 | 月間リクエスト | 推奨構成 | 月額コスト概算 | 主要サービス |
|------|--------------|---------|-------------|------------|
| **Small** | ~3,000 (100/日) | Serverless | $60-180 | Lambda + Bedrock (複数モデル) + S3 |
| **Medium** | ~30,000 (1,000/日) | Hybrid | $400-1,000 | ECS Fargate + Bedrock + DynamoDB |
| **Large** | 300,000+ (10,000/日) | Container | $2,500-6,000 | EKS + Karpenter + マルチモデル |

**Small構成の詳細** (月額$60-180):
- **Lambda**: UniRouteルーター + クラスタ選択（1GB RAM, $25/月）
- **Bedrock**: Claude 3.5 Haiku + Sonnet + Nova Lite 3モデル構成（$100/月）
- **S3**: 特徴ベクトル・代表プロンプト結果ストレージ（$5/月）
- **DynamoDB**: モデル特徴ベクトルキャッシュ（On-Demand, $10/月）
...
```hcl
resource "aws_lambda_function" "uniroute_router" {
  filename      = "uniroute.zip"
  function_name = "uniroute-model-router"
  role          = aws_iam_role.uniroute_lambda.arn
  ...
  environment {
    variables = {
      FEATURE_VECTORS_TABLE = aws_dynamodb_table.model_features.name
...
resource "aws_dynamodb_table" "model_features" {
  name         = "uniroute-model-features"
...
resource "aws_cloudwatch_metric_alarm" "routing_accuracy" {
  alarm_name          = "uniroute-quality-drop"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 3
  metric_name         = "RoutingQualityScore"
  namespace           = "UniRoute"
  period              = 3600
  statistic           = "Average"
  threshold           = 0.75
  alarm_description   = "ルーティング品質スコア低下（モデル再評価を検討）"
}
```

### コスト最適化チェックリスト

- [ ] 品質閾値$\tau$を下げてCostモード実現（5-6%品質差許容）
- [ ] 小型モデル（Haiku, Nova Lite）をクラスタ最優先で配置
- [ ] Bedrock Batch API: 非リアルタイム処理で50%割引
- [ ] Prompt Caching: システムプロンプト固定で30-90%削減
- [ ] EC2 Spot Instances: 特徴ベクトル計算で最大90%削減
- [ ] Reserved Instances: 安定ワークロードに1年コミット
- [ ] AWS Budgets: 月額予算80%で警告
- [ ] CloudWatch: モデル別品質スコア・コスト監視
- [ ] 代表プロンプト評価の定期実行（月次）でモデル特徴ベクトル更新
- [ ] Lambda: メモリサイズ最適化
- [ ] DynamoDB: TTL設定で古い特徴データ自動削除
- [ ] タグ戦略: モデル別・クラスタ別でコスト可視化
