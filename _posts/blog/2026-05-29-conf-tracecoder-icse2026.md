...
著者らが紹介する具体例として、「正の数のみをフィルタリングする」コードのバグがあります。初期コードは`x >= 0`と記述されていましたが、正しくは`x > 0`（0を除外）でした。

- **従来手法**: テスト失敗のassertionエラーのみが見え、原因不明
- **TraceCoder**: 計装により `"condition 0 >= 0 is True. Appending"` というトレースが出力され、分析エージェントが0の包含を論理エラーとして特定し、`x > 0`への修正を生成

このように、実行トレースは「何が起きたか」を可視化し、pass/failでは得られない因果情報を提供します。
