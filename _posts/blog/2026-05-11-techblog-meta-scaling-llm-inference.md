...
テンソル並列における通信オーバーヘッドをより具体的に理解するために、AllReduceの帯域幅要件を計算する。

1回のAllReduce通信で転送されるデータ量は以下のとおりである。

$$
\text{AllReduce Data} = 2 \times (N - 1) / N \times D \times \text{sizeof(dtype)}
$$

ここで $N$ はGPU数、$D$ はテンソルの要素数である。例えばLlama 3 70B（hidden dim = 8192）のTP=8構成では、各レイヤーのAllReduceで約128KBのデータ転送が発生する。80レイヤー分で約10MB/ステップとなる。

PCIe Gen4 x16の理論帯域幅は約32GB/sであるのに対し、NVLinkは最大900GB/s（H100）であり、この帯域幅の差がスケーリング効率の差に直結する。MetaのDDAはソフトウェア側のオーバーヘッド削減でPCIeでも改善余地を示しているが、物理的な帯域幅の制約は残る。

### Docker Compose環境への示唆

Zenn記事のDocker Compose構成（複数Ollamaインスタンス + Nginx）は、テンソル並列ではなくデータ並列（各インスタンスが独立にリクエストを処理）に近い構成である。Metaの分析に基づくと、この構成は以下の特徴を持つ。

- **Prefillフェーズ**: 各インスタンスが独立に処理するため、GPU間通信が不要。テンソル並列のAllReduceオーバーヘッドが発生しない
- **Decodeフェーズ**: 同様に独立処理。メモリ帯域は各GPU内で完結する
- **トレードオフ**: 単一モデルのレイテンシは改善しないが、スループット（同時処理数）は線形にスケールする

このため、レイテンシよりスループットが重要なユースケース（バッチ処理、非同期API等）では、Zenn記事のマルチインスタンス構成はMetaのテンソル並列よりも効率的な場合がある。
